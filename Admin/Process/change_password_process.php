<?php
    ...
    if(isset($_POST['changepass'])){
        ...
        if ($oldpassword != $oldpass_data['password']){ 
            header("Location: ../../indexforAdmin.php?content=ChangePassword&msg1=Mật khẩu cũ không trùng khớp");  
        }else{
            if($pwdconfirm != $pwd){
                header("Location: ../../indexforAdmin.php?content=ChangePassword&msg1=Mật khẩu không trùng khớp");
            } else{
                $sql_update = "UPDATE staff SET password = '$pwd' where phone = '$admin_phone'";
                mysqli_query($mysqli,$sql_update);
                header("Location: ../../indexforAdmin.php?content=ChangePassword&success=Thành công");
            }
        }
    }
